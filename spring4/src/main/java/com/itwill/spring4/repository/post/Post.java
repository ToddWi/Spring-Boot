...
@Table(name = "POSTS") // 엔터티 클래스(Post) 이름이 데이터베이스 테이블 이름(POSTS)과 다른 경우 name 속성으로 테이블 이름을 매칭시켜주어야 함.
@SequenceGenerator(name = "POSTS_SEQ_GEN", sequenceName = "POSTS_SEQ", allocationSize = 1)
public class Post extends BaseTimeEntity{
// 여기서는 테이블의 이름과 클래스 이름을 일치시켜주지 않음. 스프링부트에서의 post 클래스는 테이블 POSTS를 구성하는 여러개의 post 클래스 객체중 하나로
// 생각할 것이기 때문
